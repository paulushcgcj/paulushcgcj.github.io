...
## Productivity Train
To gain productivity, you must first understand what is expected of you as an output. Here is an example based on a junior developer vs. a lead developer. Remember that this is only an example and cannot be considered as a law written in stone. 
...
One suggestion gathered from people that work with a lot of tasks in parallel is to keep notes. For example, when coding, start by planning what are the steps needed to achieve what is required, take notes in your code using comments as if it were a notepad, and express what is expected for that piece of code once it is done.
